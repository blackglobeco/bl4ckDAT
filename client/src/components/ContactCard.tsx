...
export function ContactCard({
    ...
    return (
        <div className="bg-gray-900 rounded-xl shadow-lg overflow-hidden border border-gray-800">
            {/* Header */}
            <div className="bg-gradient-to-r from-grey-700 to-grey-800 p-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <span className={clsx(
                            "px-2 py-1 rounded text-xs font-medium flex items-center gap-1",
                            platform === 'whatsapp' ? "bg-green-100 text-green-700" : "bg-grey-100 text-blue-700"
                        )}>
                            <MessageCircle size={12} />
                            {platform === 'whatsapp' ? 'WhatsApp' : 'Signal'}
                        </span>
                        <h3 className="text-lg font-semibold text-gray-100">{blurredNumber}</h3>
                    </div>
                    <button
                        onClick={onRemove}
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-gray-900 p-6 rounded-xl shadow-sm border border-gray-800 flex flex-col items-center text-center">
                    <div className="relative mb-4">
                        <div className="w-32 h-32 rounded-full overflow-hidden bg-gray-700 border-4 border-gray-800 shadow-md">
                            {profilePic ? (
                                <img
                                    src={profilePic}
                                    alt="Profile"
                        ...
                        <div className={clsx(
                            "absolute bottom-2 right-2 w-6 h-6 rounded-full border-2 border-gray-800",
                            currentStatus === 'OFFLINE' ? "bg-red-500" :
                                currentStatus.includes('Online') ? "bg-green-500" : "bg-gray-500"
                        )} />
                    </div>

                    <h4 className="text-xl font-bold text-gray-100 mb-1">{blurredNumber}</h4>

                    <div className="flex items-center gap-2 mb-4">
                        <span className={clsx(
                            "px-3 py-1 rounded-full text-sm font-medium",
                            currentStatus === 'OFFLINE' ? "bg-red-100/20 text-red-400" :
                                currentStatus.includes('Online') ? "bg-green-100/20 text-green-400" :
                                    currentStatus === 'Standby' ? "bg-yellow-100/20 text-yellow-400" : "bg-gray-100/20 text-gray-400"
                        )}>
                            {currentStatus}
                        </span>
                    </div>

                    <div className="w-full pt-4 border-t border-gray-700 space-y-2">
                                ...
                                {devices.map((device, idx) => (
                                    <div key={device.jid} className="flex items-center justify-between text-sm py-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-gray-400">Device {idx + 1}</span>
                                        </div>
                                        <span className={clsx(
                                            "px-2 py-0.5 rounded text-xs font-medium",
                                            device.state === 'OFFLINE' ? "bg-red-100/20 text-red-400" :
                                                device.state.includes('Online') ? "bg-green-100/20 text-green-400" :
                                                    device.state === 'Standby' ? "bg-yellow-100/20 text-yellow-400" : "bg-gray-100/20 text-gray-400"
                                        )}>
                                            {device.state}
                                        </span>
                                    </div>
                ...
                <div className="md:col-span-2 space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-700">
                            <div className="text-sm text-gray-400 mb-1 flex items-center gap-1"><Activity size={16} /> Current Avg RTT</div>
                            <div className="text-2xl font-bold text-white">{lastData?.avg.toFixed(0) || '-'} ms</div>
                        </div>
                        <div className="bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-700">
                            <div className="text-sm text-gray-400 mb-1">Median (50)</div>
                            <div className="text-2xl font-bold text-white">{lastData?.median.toFixed(0) || '-'} ms</div>
                        </div>
                        <div className="bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-700">
                            <div className="text-sm text-gray-400 mb-1">Threshold</div>
                            <div className="text-2xl font-bold text-blue-400">{lastData?.threshold.toFixed(0) || '-'} ms</div>
                        </div>
                    </div>

                    {/* Chart */}
                    <div className="bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-700 h-[300px]">
                        <h5 className="text-sm font-medium text-gray-300 mb-4">RTT History & Threshold</h5>
